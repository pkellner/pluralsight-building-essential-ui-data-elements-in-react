...
function NotesModalTags() {
  const { modalNoteTagIds, setModalNoteTagIds, tagNamesNewValue, setTagNamesNewValue } =
            ...
            <div className="mr-2 container">
              <div className="row">
                {tagsData ? [...tagsData].sort(function (a, b) {
                  ...
                  return (
                    <div className="col-2 margin-left-right-15" key={rec.id}>
                      <input className="form-check-input" type="checkbox" id={`formchecklabelid-${rec.id}`}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setModalNoteTagIds([...modalNoteTagIds, rec.id,]);
                          } else {
                            setModalNoteTagIds(modalNoteTagIds.filter(r => r != rec.id));
                          }
                        }}
                        checked={modalNoteTagIds.includes(rec.id) ? true : false} />
                      <label className="form-check-label"
                        htmlFor={`formchecklabelid-${rec.id}`}>{rec.tagName}</label>
                    </div>
                  );
                }) : null }
              </div>
